...
After the queue-order fix, the clean no-intercept fixture passed 3/3. The latest rerun produced a `3160672`-byte ATT file plus fresh UI output.
...
- `unit.thread_trace.resource_deinit_drains_active_device_trace`: passed.
- Producer/consumer suite: 10/10.
- Earlier focused quick-scan tests across gfx11, gfx12, MI400, gfx9, chunk state, event mapping, and API errors: 9/9.
- The full rocprof-trace-decoder tree passed 1,133/1,133 tests, including regular, ASan, UBSan, logging, API-lifetime, and gfx11 quick-scan coverage.
- The standalone AQLprofile tree passed all 90 enabled tests; `SqttBuilderTest.BufferStepCalculation` remained disabled.
...
Result: 2/2 passed. The latest official run produced a `654752`-byte ATT payload with decoded in-region data. A focused installed-runtime run with a 1 MiB pool and two SEs wrote `397280` bytes and produced decoded UI output.
...
Result: execution and validation passed 2/2. The latest run produced a `654752`-byte ATT payload.
...
The registered `rocprofv3 --att --att-consecutive-kernels 8 ... vector-ops` command was run directly from the fixture working directory with `HSA_DISABLE_XDNA=1`, followed by the registered `validate.py::test_csv_data` pytest command and its original environment. The execution exited normally, wrote a `5307072`-byte ATT stream, and the validator passed 1/1.

### Detailed triple buffer

- The four enabled API fixtures, CTest tests 480, 481, 482, and 484, passed 4/4: consistency/`NO_DETAIL`, detailed hammer, slow-consumer/`NO_DETAIL`, and repeated start/stop.
- The consistency/`NO_DETAIL` fixture wrote `3161600` bytes.
- The detailed hammer fixture wrote `3068864` bytes and generated decoded UI output.
- Producer/consumer coverage passed 10/10.
...
Result: clean setup, execute, and validation passed 3/3. The latest fixture produced a `3160672`-byte ATT file and a populated `ui_output_agent_*` directory.
