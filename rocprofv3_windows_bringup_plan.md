...
- the common 469-entry gfx1151 catalog: 255 raw counters, 155 derived metrics, and 59 agent constants;
...
Completion delivery is a Windows-specific boundary. Linux hands each profiler-owned completion signal to ROCr, where the driver signals the registered event and the async-events thread runs the handler. On native Windows the same registrations are serviced by that thread's fallback polling, which naps between scans and is stopped when the runtime shuts down - and the profiler finalizes after that. An application that exits immediately after its last dispatch therefore outlives the observation and loses the record. The Windows queue instead tracks every profiler-owned completion signal and polls it from a dedicated process-lifetime waiter thread with a 100 us per-signal wait slice, so completion is observed while the profile resources, and therefore the counter data, are still alive. Registrations are claimed exactly once, which keeps session data, pooled-signal release, and the queue's active-kernel count balanced.
...
size: 448280
MD5:  f1a4636bfc6eadb6e9a20f9681bfff1c
```
...
- the 469-entry gfx1151 catalog matched the expected metadata contract, with positive installed collection for the qualified representative counters;
