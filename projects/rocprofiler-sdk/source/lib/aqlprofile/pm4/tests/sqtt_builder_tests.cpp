...
struct TestGfx115xPrimitives
{
    ...
    static constexpr uint32_t TT_LOCKDOWN_FAIL        = 0x10;
...
public:
    ...
    void BuildCopyRegDataPacket(CmdBuffer*, const Register& reg, const void* dst, uint32_t, bool wait)
    {
...
public:
    ...
    void ReadValues(CmdBuffer* cmd_buffer, const TraceConfig* config, size_t se_index)
    {
        ...
        builder_.BuildCopyRegDataPacket(cmd_buffer,
...
TEST_F(SqttBuilderTest, BufferStepCalculation)
{
    ...
    const uint64_t disabled_se_capacity = 1ULL << TestPrimitives::TT_BUFF_ALIGN_SHIFT;

    // Test case 1: All SEs enabled (4 SEs). The builder reserves minimal capacity
    // for disabled SEs and rounds each enabled SE slice down to the SQTT alignment.
    uint64_t mask1 = 0xF;  // 0b1111
    uint64_t step1 = builder.GetBaseStep(total_buffer, mask1);
    EXPECT_LE(step1 * builder.PopCount(mask1),
              total_buffer - (64 - builder.PopCount(mask1)) * disabled_se_capacity);
    ...
    EXPECT_LE(step2 * builder.PopCount(mask2),
              total_buffer - (64 - builder.PopCount(mask2)) * disabled_se_capacity);
