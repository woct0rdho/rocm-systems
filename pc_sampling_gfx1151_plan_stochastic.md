...
- `HSA_DISABLE_XDNA=1 python3 test_stochastic_sampling.py` passed with `24,330` decoded rows, `2,271` issued rows, populated instruction/reason metadata, and expected all-zero gfx1151 `Wave_Count` where the hardware register is unavailable.
