...
// Dispatch completions observed by the queue's own waiter thread.
//
// The Windows queue does not hand dispatch-completion signals to the HSA async-signal
// machinery: ROCr delivers those handlers from its async-events thread, which polls with a
// backoff nap and is torn down before the profiler finalizes, so a dispatch that completes
// right before the application stops submitting can be missed. Registering each dispatch here
// lets a dedicated thread observe completion with microsecond latency while the profile
// resources are still alive, which is what the counter data needs.
struct pending_completion
{
    hsa_signal_t signal = null_signal;
    void*        data   = nullptr;
};

std::mutex&
pending_completions_mutex()
{
    static auto* value = new std::mutex{};
    return *value;
}

// Timeout used by the completion waiter thread for a single poll of one pending signal.
constexpr auto kCompletionPollSliceNs = uint64_t{100000};

std::vector<pending_completion>&
pending_completions()
{
    static auto* value = new std::vector<pending_completion>{};
    return *value;
}

void
register_pending_completion(hsa_signal_t signal, void* data)
{
    auto lock = std::lock_guard<std::mutex>{pending_completions_mutex()};
    pending_completions().push_back(pending_completion{signal, data});
}

// Removes the registration for |data| and reports whether the caller now owns it. Exactly one
// caller can claim a registration, so the completion handler always runs at most once and the
// session data is released exactly once.
bool
claim_pending_completion(void* data)
{
    auto lock = std::lock_guard<std::mutex>{pending_completions_mutex()};
    auto& entries = pending_completions();
        ...
        if(packet.is_serialized)
        {
            CHECK_NOTNULL(get_queue_controller())
                ...
                .wlock([&](auto& serializer) {
                    serializer.kernel_completion_signal(session.queue, packet.is_serialized);
                });
        }
...
// Deliver every tracked completion whose signal has reached the dispatch-completion value.
// Only used by the completion waiter thread: late delivery from the SDK's own teardown path
// would run the SDK callbacks after the profiler services have started shutting down.
void
poll_pending_completions(const CoreApiTable& core)
{
    auto entries = std::vector<pending_completion>{};
    {
        auto lock = std::lock_guard<std::mutex>{pending_completions_mutex()};
        entries   = pending_completions();
    }

    for(const auto& entry : entries)
    {
        if(!entry.data || entry.signal.handle == 0) continue;

        if(core.hsa_signal_load_relaxed_fn != nullptr &&
           core.hsa_signal_load_relaxed_fn(entry.signal) != -1)
        {
            if(core.hsa_signal_wait_relaxed_fn == nullptr) continue;
            core.hsa_signal_wait_relaxed_fn(entry.signal,
                                            HSA_SIGNAL_CONDITION_EQ,
                                            -1,
                                            kCompletionPollSliceNs,
                                            HSA_WAIT_STATE_BLOCKED);
            if(core.hsa_signal_load_relaxed_fn(entry.signal) != -1) continue;
        }

        if(claim_pending_completion(entry.data)) completion_handler(-1, entry.data);
    }
}

// Dispatch completions have to be observed while the application is still running: the
// counter profile data is only decodable while the profile resources are alive, and the
// application can exit immediately after its last dispatch. The thread is a process-lifetime
// service; each scan waits up to |kCompletionPollSliceNs| on every pending signal before
// moving on.
void
completion_waiter_thread(CoreApiTable core)
{
    for(;;)
    {
        poll_pending_completions(core);

        auto has_pending = false;
        {
            auto lock   = std::lock_guard<std::mutex>{pending_completions_mutex()};
            has_pending = !pending_completions().empty();
        }
        if(!has_pending) std::this_thread::yield();
    }
}
...
{
    register_pending_completion(raw_signal, data);

    // Start the per-process completion waiter on first use. It delivers completions with
    // microsecond-scale latency instead of waiting for ROCr's backoff-polled async-events
    // thread, which the application can outrun when it exits right after its last dispatch.
    static auto* waiter_started = new std::atomic<bool>{false};
    if(!waiter_started->exchange(true, std::memory_order_acq_rel))
    {
        auto core_table = _core_api;
        std::thread([core_table]() { completion_waiter_thread(core_table); }).detach();
    }
...
Queue::sync() const
{
    if(_active_kernels.handle == 0 || !_core_api.hsa_signal_wait_relaxed_fn) return true;
