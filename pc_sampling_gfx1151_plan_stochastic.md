...
- `unit.pcs_parser` passed `16/16`; the SDK PC-sampling groups passed `34/34` including upstream's `unit.pc_sampling_versioning` cases.
- ROCr lifecycle/config tests passed `2/2`, and KFD PC-sampling validation passed all `5/5` enabled cases.
- `HSA_DISABLE_XDNA=1 python3 test_stochastic_sampling.py` passed with `24,343` decoded rows, `2,413` issued rows, populated instruction/reason metadata, and expected all-zero gfx1151 `Wave_Count` where the hardware register is unavailable.
...
- The rebuilt installed runtime used for these checks reported version `7.16.26374-f39ab5add8` after rebasing onto the current `upstream/develop`.
