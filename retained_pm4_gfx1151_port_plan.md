...
Upstream moved HRR playback out of CLR into the standalone `projects/hrr` tree during this rebase, so the fork's `projects/clr/hipamd/src/hrr/playback/CMakeLists.txt` hsa-runtime64 link change was dropped with the file; the standalone tree links `hip::host` and names `hsa-runtime64` only where a test calls ROCr directly.
...
- ROCr `graph_command_encoder_test`: `6/6` tests passed (`Gfx11RejectsUnsupportedAbiShapes`, `QueueScratchUsesPatchableTmpRingState`, `CapabilityTableQualifiesOnlyGfx1151`, `DynamicLdsIsEncodedAndOverflowRejected`, and the encoder/ABI cases).
...
- The rebase rerun against HIP `7.16.26374-f39ab5add8` and the current ROCr source repeated the graph checks: `graph_command_encoder_test` passed `6/6`, the HIP graph-exec tests reported `27` passed / `1` skipped with `235` assertions, and the seven `Performance_Graph_*` cases passed with `354` assertions.
...
- Binary: `~/llama.cpp/build/bin/llama-bench`, llama.cpp `934acccb2` (`0.5.0-dev`, build 11212).
- Runtime: fork-patched HIP `7.16.26374-f39ab5add8` built from `~/rocm-systems` `16b938f6e1`.
- Arguments: `-ngl 99 -fa on -b 2048 -ub 512 -dev ROCm0` with five repetitions per workload.
...
| --- | ---: | ---: | ---: |
| `pp512` | 1468.81 +/- 19.46 | 1501.70 +/- 33.29 | +2.24% |
| `pp2048` | 1457.52 +/- 5.20 | 1472.71 +/- 8.81 | +1.04% |
| `tg128` | 58.82 +/- 0.08 | 69.16 +/- 0.12 | +17.58% |
| `tg512` | 58.88 +/- 0.05 | 69.38 +/- 0.03 | +17.83% |
...
```text
[hipGraph][PM4] retained 2954 dispatches in 85867 dwords, max private 1260 bytes
[hipGraph][PM4] retained 1704 dispatches in 51038 dwords, max private 24 bytes
[hipGraph][PM4] retained 1644 dispatches in 49358 dwords, max private 24 bytes
```
...
- Model: `DeepSeek-V4-Flash-Vision-Exp-IQ2XXS.gguf`, IQ2_XXS, 284.3B parameters, 81.0 GiB.

| Workload | PM4 disabled | PM4 enabled | Change |
| --- | ---: | ---: | ---: |
| `pp512` | 246.40 +/- 1.24 | 247.52 +/- 1.71 | +0.45% |
| `pp2048` | 237.05 +/- 0.50 | 237.05 +/- 0.38 | 0.00% |
| `tg128` | 16.00 +/- 0.01 | 18.25 +/- 0.04 | +14.06% |
| `tg512` | 15.94 +/- 0.00 | 18.11 +/- 0.07 | +13.61% |

A diagnostic run with PM4 enabled and `AMD_LOG_LEVEL=3` confirmed retained execution for this larger model:

```text
[hipGraph][PM4] retained 5740 dispatches in 165320 dwords, max private 100 bytes
[hipGraph][PM4] retained 4719 dispatches in 134994 dwords, max private 0 bytes
```

The corresponding generation run completed with successful HIP graph launches. As with Qwen3.6, PM4 provides a large generation improvement while prompt processing is effectively unchanged. This result strengthens the case for enabling `DEBUG_HIP_GRAPH_PM4=1` in the target llama.cpp environment, while retaining the runtime's opt-in default.

### Qwen3.8-Flash-Next Workload Benchmark

A third benchmark used the local `gfx1151` Radeon 8060S and:
- Model: `Qwen3.8-Flash-Next-IQ4_NL.gguf`, IQ4_NL, 176.94B parameters, 93.16 GiB.

| Workload | PM4 disabled | PM4 enabled | Change |
| --- | ---: | ---: | ---: |
| `pp512` | 931.16 +/- 32.38 | 926.24 +/- 120.70 | -0.53% |
| `pp2048` | 940.91 +/- 4.51 | 946.49 +/- 9.14 | +0.59% |
| `tg128` | 30.54 +/- 0.09 | 36.42 +/- 0.13 | +19.25% |
| `tg512` | 30.62 +/- 0.02 | 36.50 +/- 0.03 | +19.20% |

A diagnostic run with PM4 enabled and `AMD_LOG_LEVEL=3` confirmed retained execution for this model:

```text
[hipGraph][PM4] retained 3835 dispatches in 106505 dwords, max private 8 bytes
[hipGraph][PM4] retained 3582 dispatches in 104023 dwords, max private 0 bytes
[hipGraph][PM4] retained 3509 dispatches in 102012 dwords, max private 0 bytes
```

The `pp512` figure is noisy for this model (repeat runs ranged from about 904 to 936 t/s), so `pp2048` is the reliable prefill measure. As with the two earlier models, PM4 provides a large generation improvement while prompt processing is effectively unchanged. The three-model result strengthens the case for enabling `DEBUG_HIP_GRAPH_PM4=1` in the target llama.cpp environment, while retaining the runtime's opt-in default.

### DeepSeek V4 Loading Mode

Historically the DeepSeek benchmark used `--load-mode dio` because that was the previously established conservative setting for this unified-memory machine. A guarded probe using the default `--load-mode auto` also loaded the full model successfully without an OOM or duplicate model allocation. The loader reported:
...
For the current HIP build, `auto` disables mmap for the integrated `gfx1151` device and resolves to ordinary loading (`none`). Therefore `--load-mode dio` is no longer required for correctness or to avoid the mmap/file-cache duplication problem, and current benchmarks use the default load mode. Explicit `--load-mode mmap` should not be used on this machine.
