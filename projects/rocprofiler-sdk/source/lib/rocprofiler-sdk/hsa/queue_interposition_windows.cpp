...
// The Windows queue owns completion observation: its waiter thread is started on demand by the
// first registered completion and retires completions as they arrive, so the interposition
// layer has no monitor of its own to start or join. queue_controller_sync has already waited
// for the queue to drain when stop_completion_monitor is called, so no stragglers are left to
// retire here either.
void
